...
const ExistingSchedule = (props) => {
  ...
  const get_table_contents = (time) => {
    const elts_of_week = [
      <td>
        {0 === parseInt(time.substring(time.indexOf(":") + 1)) ? convert_time_to_ampm(time) : ""}
      </td>,
    ...
    for (const evt of schedEvents) {
      if (
        evt.scheduleName === scheduleName &&
        evt.startHour === parseInt(time.substring(0, time.indexOf(":"))) &&
        evt.startMinute === parseInt(time.substring(time.indexOf(":") + 1))
      ) {
        var correct_index = dayToNum(evt.day) + 1;
        for (const week of univ_table) {
          for (const week_evt of week) {
            if (
              dayToNum(week_evt.day) < dayToNum(evt.day) &&
              convert_time_to_num(week_evt.endHour, week_evt.endMinute) >
                convert_time_to_num(evt.startHour, evt.startMinute)
            ) {
              correct_index--;
              elts_of_week.pop();
            }
